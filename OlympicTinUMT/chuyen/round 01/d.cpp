...
signed main() {
    ...
    for (auto i : ma) a_cnt *= fact[i.second % MOD] % MOD, a_cnt %= MOD;

    sort(a.begin(), a.end(), cmpB);
    if (check(a)) {
        ab_cnt = 1;
        for (auto i : myMap) ab_cnt *= fact[i.second % MOD] % MOD, ab_cnt %= MOD;
    }
    for (auto i : a) mb[i.second % MOD]++;
    for (auto i : mb) b_cnt *= i.second % MOD, b_cnt %= MOD;
    int ans = (fact[n] % MOD - a_cnt % MOD - b_cnt % MOD + ab_cnt % MOD) % MOD;
    cout << ans;
    return 0;
}