...
inline          int         compute_sum(int n) { int res = 0; int combination = 1; for (int i = 1; i <= n; ++i) { combination = combination * (n - i + 1) % MOD * bpow(i, MOD - 2, MOD) % MOD; int term = combination * bpow(i, 2024, MOD) % MOD; res = (res + term) % MOD;}return res;}
...
int numb[100005], low[100005];
int timeDfs = 0;
bool deleted[100005] = { false };
stack<int> mySt;
int stronglyCount = 0;
...
void dfs(int u) {
	...
	for (auto v : adj[u]) {
		...
		if (numb[v] != 0) low[u] = min(low[u], numb[v]);
		else {
			dfs(v);
